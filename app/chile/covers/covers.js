    ...
    const loadData = async () => {
      ...
      if (storedDb) {
        try {
          ...
          try {
            await AsyncStorage.setItem("chileDB", JSON.stringify(normalized));
          } catch (e) {
            console.warn("No se pudo sobrescribir chileDB tras migración:", e);
          }

          setData(normalized);
        } catch (e) {
          console.warn("Error parseando chileDB:", e);
          setData(inventory);
        }
      }
    };
